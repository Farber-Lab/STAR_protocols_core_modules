...
load("./data/expression_matrix/exp_mat.Rdata")
...
mono_results = as.data.frame(matrix(nrow=74,ncol=3))
...
phen_results = as.data.frame(matrix(nrow=74,ncol=3))
...
for (i in 1:length(gene_files)){
  print(i)
  x = gene_files[i] 
  y = nchar(gene_files[i]) - 23 # the 23 is so that the text at the end of the file name is cut off and just the 
                                # tissue is left. You many need to change it to match whatever you named your files
                                # mine looked like "Adipose_Subcutaneous_b4galnt3_ebmd_snps.txt"
  tissue = str_sub(x, 1, y)     # This will cut the extra characters so just the tissue is left
  z = read_tsv(paste0("./data/eqtl_data/b4galnt3_snps/",x), col_names = FALSE) # this will read in the eqtl file
  # formatting
